...
namespace DataMesh.AR.Library
{
    ...
    public static class LibraryAssetMaker
    {

        [MenuItem("Assets/DataMesh/Build Library Asset (PC)")]
        public static void MakeAssetPC()
        {
            MakeAsset(BuildTarget.StandaloneWindows64);
        }

        [MenuItem("Assets/DataMesh/Build Library Asset (WSA)")]
        ...
        private static void MakeAsset(BuildTarget buildTarget)
        {
            ...
            for (int i = 0; i < Selection.objects.Length; i++)
            {
                ...
                if (sel != null && sel is GameObject)
                {
                    ...
                    LibraryAssetManager.EncryptAsset(obj.name.ToLower(), srcPath, destPath, assetPath);
